...
export function CodeThemeSelector() {
  ...
  return <Popover open={open} onOpenChange={setOpen}>
      ...
      <PopoverContent align="end" className="w-48 p-2">
        ...
        {codeThemes.map(t => <button key={t.id} onClick={() => {
        setTheme(t.id);
        setOpen(false);
      }} className={`w-full flex items-center justify-between p-2 rounded-lg hover:bg-secondary transition-colors ${theme === t.id ? "bg-secondary" : ""}`}>
